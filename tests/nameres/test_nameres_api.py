...
def test_reverse_lookup(target_info):
    ...
    response = requests.post(reverse_lookup_url, json={"curies": curies_should_not_work + curies_should_work})
    assert response.ok
    ...
    response = requests.get(reverse_lookup_url, params={"curies": curies_should_not_work + curies_should_work})
    assert response.ok
    response_json = response.json()
