...
def load_blocklist_from_gsheet(google_sheet_id):
    """
...
def test_check_blocklist_entry(target_info):
    """
    ...
    blocklist_gsheet_id = target_info['BlocklistGSheetID']

    # We only support the Google Sheet blocklist for now.
    if not blocklist_gsheet_id:
        return

    blocklist_entries = load_blocklist_from_gsheet(blocklist_gsheet_id)

    assert len(blocklist_entries) > 0, f"No blocklist entries found in Google Sheet {blocklist_gsheet_id}"
    ...
    for entry in blocklist_entries:
        ...
        if not entry.is_blocked():
            logging.info(f"Skipping blocklist entry as it is not asserted to be blocked: {entry}")
            continue

        assert entry.CURIE, f"Blocklist entry claims to be blocked, but does not have a CURIE: {entry}"
        ...
        assert response.ok
        result = response.json()[entry.CURIE]

        assert result == {}, f"Expected {entry.CURIE} to be absent on {nameres_url_reverse_lookup}, but found: {result}"
