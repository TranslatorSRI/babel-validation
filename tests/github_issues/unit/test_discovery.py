...
from src.babel_validation.sources.github.github_issues_test_cases import issue_id
...
class TestGetIssuesByIdsNotFound:
    ...
    def _missing(self, github_issues_test_cases):
        repo = MagicMock()
        repo.get_issue.side_effect = UnknownObjectException(404, {"message": "Not Found"}, {})
        return patch.object(github_issues_test_cases.github, "get_repo", return_value=repo)
    ...
    def test_missing_issue_raises_value_error(self, github_issues_test_cases, issue_ref):
        with self._missing(github_issues_test_cases):
            with pytest.raises(ValueError, match="Could not resolve issue ID"):
                github_issues_test_cases.get_issues_by_ids([issue_ref])

    def test_found_issue_is_returned(self, github_issues_test_cases):
        repo, issue = MagicMock(), MagicMock()
        repo.get_issue.return_value = issue
        with patch.object(github_issues_test_cases.github, "get_repo", return_value=repo):
            assert github_issues_test_cases.get_issues_by_ids(["test-org/test-repo#1"]) == [issue]

    def test_unparseable_id_raises_value_error(self, github_issues_test_cases):
        with pytest.raises(ValueError, match="Could not resolve issue ID"):
            github_issues_test_cases.get_issues_by_ids(["not-an-issue-reference"])
...
class TestConfiguredRepositoriesOnly:
    ...
    Here rather than with the other untrusted-input guards because it and
    TestGetIssuesByIdsNotFound cover the same method, and keeping both beside get_issues_by_ids
    is worth more than the thematic grouping.
    """
    ...
    def test_unconfigured_repo_is_never_fetched(self, github_issues_test_cases, issue_ref):
        with patch.object(github_issues_test_cases.github, "get_repo") as get_repo:
            with pytest.raises(ValueError, match="Could not resolve issue ID"):
                github_issues_test_cases.get_issues_by_ids([issue_ref])
